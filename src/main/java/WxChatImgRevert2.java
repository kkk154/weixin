...
public class WxChatImgRevert2 {
    ...
    public static void main(String[] args) {
        if(args.length<=2){
            System.out.println("参数不完整请重新输入");
            System.out.println("[Src Path] [Target Path]");
            return;
        }
    ...
    private static void convert(String path, String targetPath) {
        ...
        for (File file1:file) {
            ...
            if (x.get() == 0 && xor[1] != null && (Integer) xor[1] != 0) {
                x.set((Integer) xor[1]);
            }
            xor[1] = xor[1] == null ? x.get() : xor[1];
            try {
                InputStream reader = new FileInputStream(file1);
                OutputStream writer = new FileOutputStream(targetPath + File.separator + file1.getName().split("\\.")[0] + (xor[0] != null ? "." + xor[0] : ""));
                byte[] bytes = new byte[1024 * 10];
                int b;
                while ((b = reader.read(bytes)) != -1) {//这里的in.read(bytes);就是把输入流中的东西，写入到内存中（bytes）。
                    for (int i = 0; i < bytes.length; i++) {
                        bytes[i] = (byte) (int) (bytes[i] ^ (Integer) xor[1]);
                        if (i == (b - 1)) {
                            break;
                        }
                    }
